import { _decorator, Component, Node, systemEvent, SystemEvent, EventKeyboard, macro, Vec3, Quat, quat, v3, tween, geometry, director, SkeletalAnimationComponent, Terrain, find, AnimationState, AnimationClip, SkinningModelComponent, v4, Vec4 } from 'cc';
...
export class PlayerCtrl extends Component {
    ...
    public set isLoop(v: boolean) {
        this._isLoop = v;
        let animState = this._anim.getState(this.curAnimName) //  this._anim.getAnimationState(this.curAnimName);
    ...
    playAnim(animName, isLoop?: boolean, cb?: Function) {
        let animState =this._anim.getState(this.curAnimName) // this._anim.getAnimationState(animName);
